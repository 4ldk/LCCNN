...
from utils import fpr_score, pv_score, recall_score
...
class Net(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pv = pv_score(label, pred, average="macro").tolist()
        fpr = fpr_score(label, pred, average="macro").tolist()

        return {"acc": acc, "recall": recall, "pv": pv, "fpr": fpr}
    ...
    def validation_epoch_end(self, outputs):
        ...
        ave_pv = torch.tensor([x["pv"] for x in outputs]).to(torch.float).mean()
        ave_fpr = torch.tensor([x["fpr"] for x in outputs]).to(torch.float).mean()

        self.log("acc", ave_acc)
        self.log("recall", ave_recall)
        self.log("pv", ave_pv)
        self.log("fpr", ave_fpr)
